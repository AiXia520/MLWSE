...
# Features combine the output of each base classifier合
...
if __name__ == '__main__':
    ...
    # 5-folde cross validation

    alphas = [math.pow(10, -5), math.pow(10, -4),
             math.pow(10, -3),math.pow(10, -2),
             math.pow(10, -1),math.pow(10, 0),math.pow(10, 1),
             math.pow(10, 2),math.pow(10, 3),math.pow(10, 4)]

    betas = [math.pow(10, -5), math.pow(10, -4),
             math.pow(10, -3),math.pow(10, -2),
             math.pow(10, -1),math.pow(10, 0),math.pow(10, 1),
             math.pow(10, 2),math.pow(10, 3),math.pow(10, 4)]
    ...
    df = pd.DataFrame(columns=["alpha", "beta","Accuracy", "F1","Micro-F1","Macro-F1"])
    tune_parameter=list()
    for alpha in alphas:
        tune_parameter.append(alpha)
        for beta in betas:
            ...
            df=df.append(pd.DataFrame({'alpha':[alpha],'beta':[beta],'Accuracy':[result.iat[0,1]],'F1':[result.iat[3,1]],'Micro-F1':[result.iat[6,1]],'Macro-F1':[result.iat[7,1]]}),ignore_index=True)
                # tune_parameter.append(result.iat[0,1])
            print(df)
    # df.to_excel("stacking_gpos_go_tune_parameter.xls")
