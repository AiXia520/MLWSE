...
if __name__ == '__main__':
    ...
    lbdas = [ math.pow(10, -5),math.pow(10, -4),
             math.pow(10, -3), math.pow(10, -2),
             math.pow(10, -1), math.pow(10, 0),
    ...
    df = pd.DataFrame(columns=["alpha","lbda" ,"beta", "Accuracy", "F1", "Micro-F1", "Macro-F1"])
    tune_parameter = list()
    for alpha in alphas:
        ...
        for lbda in lbdas:
            ...
            for beta in betas:
                ...
                # print(result)
                df=df.append(pd.DataFrame({'alpha':[alpha],'lbda':[lbda],'beta':[beta],'Accuracy':[result.iat[0,1]],'F1':[result.iat[3,1]],'Micro-F1':[result.iat[6,1]],'Macro-F1':[result.iat[7,1]]}),ignore_index=True)
                # tune_parameter.append(result.iat[0,1])
                print(df)
        df.to_excel(str(alpha)+"_stacking2_emotions_tune.xls")
    df.to_excel("stacking2_emotions_tune_parameter.xls")
